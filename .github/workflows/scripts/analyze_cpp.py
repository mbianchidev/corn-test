#!/usr/bin/env python3
"""C++ test result analyzer for corn-flakes-detection.

Parses Google Test / CTest JUnit XML reports from ``reports/``.
"""
import sys
...
def find_reports(artifacts_dir: Path) -> list[Path]:
    """Find C++ test report XML files."""
    reports = find_xml_reports(artifacts_dir, "gtest-results.xml")
    if reports:
        return reports
