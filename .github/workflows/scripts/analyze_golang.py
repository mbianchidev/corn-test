#!/usr/bin/env python3
"""Go test result analyzer for corn-flakes-detection.

Parses gotestsum JUnit XML reports from ``reports/``.
"""
import sys
...
def find_reports(artifacts_dir: Path) -> list[Path]:
    """Find Go test report XML files."""
    reports = find_xml_reports(artifacts_dir, "go-test-results.xml")
    if reports:
        return reports
